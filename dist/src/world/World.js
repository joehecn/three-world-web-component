...
import { createScene, createLights, 
// Node,
createCamera, loadBuilding, } from './components/index.js';
import { createRenderer, createControls, Resizer, ColorGUIHelper, MinMaxGUIHelper, } from './systems/index.js';
function __setScissorForElement(canvas, elem) {
...
export class World {
    constructor(canvas, mainView, secondView, controlView) {
        ...
        let aspect = 2;
        {
            const { left, positiveYUpBottom, width, height } = __setScissorForElement(this._renderer.domElement, this._mainView);
            aspect = width / height;
            this._mainCamera = createCamera([75, aspect, 1, 300] // option: [fov, aspect, near, far]
            );
        ...
        {
            this._secondView.style.height = `${this._secondView.clientWidth / aspect}px`;
            const { left, positiveYUpBottom, width, height } = __setScissorForElement(this._renderer.domElement, this._secondView);
            this._secondCamera = createCamera([120, aspect, 1, 300], // option: [fov, aspect, near, far]
        ...
        secondLightFolder
            ...
            .onChange(this.render.bind(this));
        const minMaxGUIHelper = new MinMaxGUIHelper(this._mainCamera, 'near', 'far', 0.1);
        ...
        this._scene.add(mainLight, secondLight, this._lightHelper, this._cameraHelper);
    }
    async init() {
